...
namespace ClassicUO.Game.UI.Gumps
{
    internal class SpellbookGump : Gump
    {
        ...
        // CastSpellsByOneClick already governs Assistant hotkey/macro buttons but not the
        // spellbook itself - this makes the preference actually apply here too, matching a
        // single click (not drag, not double-click) to a cast when the preference is on.
        private void OnIconClicked(object sender, MouseEventArgs e)
        {
            if (e.Button != MouseButtonType.Left || !ProfileManager.Current.CastSpellsByOneClick)
                return;

            if (Mouse.LDroppedOffset != Point.Zero)
                return;
