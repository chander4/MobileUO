...
namespace ClassicUO.Game.UI.Gumps
{
    internal class WorldMapGump : ResizableGump
    {
        ...
        public override void Update(double totalMS, double frameMS)
        {
            ...
            // Pinch-to-zoom, mirroring the pattern GameController already uses for the main
            // world view (LeanGesture pinch scale + counter threshold), applied here to this
            // gump's own zoom index instead of the scene's zoom.
            if (UnityEngine.Application.isMobilePlatform && UserPreferences.UseMouseOnMobile.CurrentValue == 0)
            {
                ...
                // MouseOverControl usually resolves to a nested child (e.g. this gump's resize
                // border), not the WorldMapGump itself - RootParent gets back to the window,
                // matching the same resolution GameController's own two-finger handling uses.
                var controlUnderFingers = UIManager.MouseOverControl?.RootParent ?? UIManager.MouseOverControl;

                if (fingers.Count == 2 && controlUnderFingers is WorldMapGump)
                {
        ...
        protected override void OnMouseDown(int x, int y, MouseButtonType button)
        {
            if (!ItemHold.Enabled)
            {
                // Touch has no Alt key and no middle mouse button, so a single-finger drag
                // (arrives here as a simulated left click) needs its own pan trigger, mirroring
                // the existing Alt+left-drag/middle-drag desktop pan gesture rather than
                // replacing it.
                bool touchPan = UnityEngine.Application.isMobilePlatform && UserPreferences.UseMouseOnMobile.CurrentValue == 0;

                if ((button == MouseButtonType.Left && (Keyboard.Alt || _freeView || touchPan)) || (button == MouseButtonType.Middle))
                {
                    if (x > 4 && x < Width - 8 && y > 4 && y < Height - 8)
                    {
                        if (button == MouseButtonType.Middle || touchPan)
                            FreeView = true;
