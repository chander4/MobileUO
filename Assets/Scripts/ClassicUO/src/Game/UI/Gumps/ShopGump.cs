    ...
    internal class ShopGump : Gump
    {
        ...
        private void ShopItem_MouseDoubleClick(object sender, MouseDoubleClickEventArgs e)
        {
            ...
            else
            {
                ...
                transactionItem.AmountLabelClicked += TransactionItem_OnAmountLabelClicked;
        ...
        // Reuses SplitMenuGump's slider+textbox pattern for direct numeric entry, instead of
        // requiring the ±1 stepper for large quantities (e.g. buying 100 arrows).
        private void TransactionItem_OnAmountLabelClicked(object sender, EventArgs e)
        {
            var transactionItem = (TransactionItem) sender;

            if (!_shopItems.TryGetValue(transactionItem.LocalSerial, out ShopItem shopItem))
                return;

            int maxAvailable = shopItem.Amount + transactionItem.Amount;

            if (maxAvailable <= 1)
                return;

            UIManager.Add(new AmountEntryGump(1, maxAvailable, transactionItem.Amount, Mouse.Position, newAmount =>
            {
                newAmount = Math.Max(1, Math.Min(newAmount, maxAvailable));
                int delta = newAmount - transactionItem.Amount;

                shopItem.Amount -= delta;
                transactionItem.Amount = newAmount;
        ...
        private class TransactionItem : Control
        {
            ...
            public TransactionItem(uint serial, ushort graphic, ushort hue, int amount, ushort price, string realname)
            {
                ...
                _amountLabel.MouseUp += (sender, e) =>
                {
                    if (e.Button == MouseButtonType.Left)
                        AmountLabelClicked?.Invoke(this, EventArgs.Empty);
                };
            ...
            public event EventHandler AmountLabelClicked;
        ...
        // Small numeric-entry popup for transaction quantity, mirroring SplitMenuGump's
        // slider+textbox sync pattern (and its visual assets) rather than requiring the
        // ±1 stepper for large quantities.
        private class AmountEntryGump : Gump
        {
            ...
            public AmountEntryGump(int min, int max, int value, Point position, Action<int> onConfirm) : base(0, 0)
            {
                ...
                Add(new GumpPic(0, 0, 0x085C, 0) { ContainsByBounds = true });
                Add(_slider = new HSliderBar(29, 16, 105, min, max, value, HSliderBarStyle.BlueWidgetNoBar));

                Button okButton;

                Add(okButton = new Button(0, 0x085d, 0x085e, 0x085f)
                {
                    ButtonAction = ButtonAction.Default,
                    X = 102, Y = 37
                });

                okButton.MouseUp += (sender, e) => Confirm();
                ...
                _textBox.SetText(value.ToString());
                _textBox.TextChanged += (sender, args) => SyncFromText();
                _textBox.SetKeyboardFocus();
                _slider.ValueChanged += (sender, args) => SyncFromSlider();
            }
            ...
            private void SyncFromText()
            {
                ...
                if (int.TryParse(_textBox.Text, out int v))
                    _slider.Value = Math.Max(_slider.MinValue, Math.Min(v, _slider.MaxValue));
            ...
            {
                _onConfirm?.Invoke(_slider.Value);
                Dispose();
            }
        }
    }
