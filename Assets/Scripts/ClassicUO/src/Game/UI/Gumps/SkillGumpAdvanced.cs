...
namespace ClassicUO.Game.UI.Gumps
{
    ...
    internal class SkillListEntry : Control
    {
        ...
        {
            // Touch-mode row: original rows are a fixed 20px, well under the ~44pt touch-target
            // convention used elsewhere in this project (e.g. ActionButtonGump). Desktop keeps the
            // compact rows; mobile gets a taller row plus padded (not stretched) hit-boxes on the
            // use/lock icons via ContainsByBounds, the same padding technique already used for
            // ShopGump's plus/minus buttons.
            bool touchMode = UnityEngine.Application.isMobilePlatform;
            int rowHeight = touchMode ? 40 : 20;
            int iconExtra = touchMode ? 16 : 0;
            int iconY = 4 + ((rowHeight - 20) >> 1);
            ...
            _skill = skill;
            skillName.X = 20;
            skillName.Y = (rowHeight >> 1) - (skillName.Height >> 1);

            if (skill.IsClickable)
            {
                Add(_activeUse = new Button((int) Buttons.ActiveSkillUse, 0x837, 0x838)
                {
                    X = 0, Y = iconY, ButtonAction = ButtonAction.Activate,
                    Width = 24 + iconExtra, Height = 24 + iconExtra,
            ...
            skillValueBase.Y = (rowHeight >> 1) - (skillValueBase.Height >> 1);

            Add(skillValueBase);
            skillValue.X = 280;
            skillValue.Y = (rowHeight >> 1) - (skillValue.Height >> 1);

            Add(skillValue);
            skillCap.X = 360;
            skillCap.Y = (rowHeight >> 1) - (skillCap.Height >> 1);

            Add(skillCap);

            GumpPic loc = new GumpPic(425, iconY, (ushort) (skill.Lock == Lock.Up ? 0x983 : skill.Lock == Lock.Down ? 0x985 : 0x82C), 0)
            {
                Width = 24 + iconExtra, Height = 24 + iconExtra,
                ContainsByBounds = true
            };
