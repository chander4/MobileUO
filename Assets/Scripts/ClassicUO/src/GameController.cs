    ...
    unsafe class GameController : Microsoft.Xna.Framework.Game
    {
        ...
        private void UnityInputUpdate()
        {
            ...
            if (UnityEngine.Application.isMobilePlatform && UserPreferences.UseMouseOnMobile.CurrentValue == 0)
            {
                ...
                //Detect two finger tap gesture for closing gumps, only on release (not on initial touch-down) and
                //only when neither finger has moved beyond a small tolerance - otherwise a two-finger pinch gesture
                //(e.g. WorldMapGump zoom) gets misread as a tap on its very first frame, before any pinch movement
                //has had a chance to happen, and fires an unwanted right-click.
                const float TWO_FINGER_TAP_MOVEMENT_TOLERANCE = 24f;

                if (fingers.Count == 2 && (fingers[0].Up || fingers[1].Up))
                {
                    bool minimalMovement =
                        UnityEngine.Vector2.Distance(fingers[0].StartScreenPosition, fingers[0].ScreenPosition) < TWO_FINGER_TAP_MOVEMENT_TOLERANCE &&
                        UnityEngine.Vector2.Distance(fingers[1].StartScreenPosition, fingers[1].ScreenPosition) < TWO_FINGER_TAP_MOVEMENT_TOLERANCE;
